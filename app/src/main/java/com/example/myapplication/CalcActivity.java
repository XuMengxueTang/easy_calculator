...
public class CalcActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btn_change).setOnClickListener(this);//改变正负
        findViewById(R.id.btn_jie).setOnClickListener(this);//阶乘
    ...
    private boolean verify(View v) {
        if (v.getId() == R.id.btn_equal) { // 点击了等号按钮
            ...
            if ((firstNum.equals("") || secondNum.equals(""))&&(operator.equals("+")||operator.equals("-")||operator.equals("×")||operator.equals("÷"))) { // 无操作数
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.btn_clear) { // 点击了清除按钮
            clear();
                ...
                || v.getId() == R.id.btn_multiply ||v.getId()==R.id.btn_divide) {
            operator = inputText; // 加减乘除运算符
            refreshText(showText + operator);

        } else if( v.getId()==R.id.btn_sin||v.getId()==R.id.btn_cos||v.getId()==R.id.btn_tan){
            operator = inputText; // 三角函数运算符
            refreshText(showText + operator);

        }
        else if(v.getId()==R.id.btn_jie){
            operator = inputText; // 阶乘运算符
            refreshText(showText + operator);
            secondNum="1";
            ...
            operator = ""; // 取反运算符
            //refreshText(showText + operator);
            double calculate_result = 0-Double.parseDouble(firstNum); // 取反
            ...
            if(operator.equals("+")||operator.equals("-")||operator.equals("×")||operator.equals("÷")){
                ...
                double calculate_result = calculateThree(); // 三角函数运算
                refreshOperate(String.valueOf(calculate_result));
                refreshText(showText + "=" + result);

            }else if(operator.equals("!")){
                double calculate_result = jiecheng(); // 三角函数运算
                refreshOperate(String.valueOf(calculate_result));
                refreshText(showText + "=" + result);
    ...
    private double calculateThree() {
        double calculate_result = 0;
        if (operator.equals("sin")) { // 当前是sin运算
            calculate_result = Math.sin(Double.parseDouble(secondNum));
        } else if (operator.equals("cos")) { // 当前是cos运算
            calculate_result = Math.cos(Double.parseDouble(secondNum));
        } else if (operator.equals("tan")) { // 当前是tan运算
            calculate_result = Math.tan(Double.parseDouble(secondNum));
        }

        Log.d(TAG, "calculate_result=" + calculate_result); // 把运算结果打印到日志中
        return calculate_result;
    }
    private double jiecheng(){
        double calculate_result = 1;
        int i=1;
        for(i=1;i<Double.parseDouble(firstNum)+1;i++){
            calculate_result=calculate_result*i;
        }
        Log.d(TAG, "calculate_result=" + calculate_result); // 把运算结果打印到日志中
        return calculate_result;
    }
    private double change(){
        double calculate_result = 0;
        int s=showText.length();
        calculate_result=0-Double.parseDouble(showText.substring(0,s-2));
        Log.d(TAG, "calculate_result=" + calculate_result); // 把运算结果打印到日志中
        return calculate_result;
    }
